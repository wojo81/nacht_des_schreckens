class ZWeapon : zmd_Weapon
{
    ...
    Property MagazineSize: clipCapacity;                    // # of rounds weapon magazine can hold
...
//         invoker.CheckMagazine(true);
    ...
    */
	...
	action State ZWL_JumpIfDepleted(StateLabel st) {
    ...
    */
    action void ZWL_Unload(int rounds = 0, bool toInventory = false)
    {
        if (rounds <= 0)
        {
            rounds = Max(invoker.clipSize + rounds, 0);
        }

        int tradeAmt = Min(rounds, invoker.clipSize);

        invoker.clipSize -= tradeAmt;
    ...
    */
    action void ZWL_Reload(int rounds = 0, bool wasteRemaining = false)
    {
        ...
        if (playerAmmo)
        {
            if (rounds <= 0)
            {
                rounds = invoker.clipCapacity - invoker.clipSize - rounds;
            }
...
//     }
    ...
    */
    override bool DepleteAmmo(bool altFire, bool checkEnough, int ammoUse)
    {
        ...
        clipSize -= ammoUse < 0 ? ammoUse1 : ammoUse;

        if (bPrimary_Uses_Both)
            ammo2.amount -= ammoUse2;

        clipSize = Max(clipSize, 0);
...
//     }
    ...
    */
    virtual bool CheckMagazine(bool autoReload, bool requireAmmo = false, int ammoCount = -1)
    {
        ...
        if (self.clipSize < (clipSize < 0 ? ammoUse1 : clipSize))
        {
...
//         bAmmo_Optional = true;
    ...
    {
        return clipSize >= clipCapacity + extraRound;
    }
